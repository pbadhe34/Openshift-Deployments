...
##################
To build the docker tomact imnage with war file addition
Build the docker image for tomcat with war file
docker build -t tomcat-deploy .
Run the tomcat insoide the docker container
docker run -d -p 8090:8080 --name tomcat tomcat-deploy
docker run -d -p 127.0.0.1:8090:8080 --name tomcat tomcat-deploy
