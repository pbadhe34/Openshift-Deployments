...
The  Docker Engine Swarm mode and Docker Swarm are two different projects, with different installation steps despite they both work in a similar way.
...
Creating network user-service_webnet
Creating service user-service_web
Creating service user-service_db
  ...
  Tasks have state and their own IDs. Let’s list the tasks:
    ...
    docker service ps user-service_web
    docker service ps user-service_db

    docker service logs user-service_db
    docker service logs user-service_web
...
the docker stack deploy command:
 ...
 Or
 docker-compose scale user-service_web=2

 docker-compose scale user-service_db=3
...
docker stack deploy -c docker-compose.yml user-app
